...
async function main() {
  console.log('🚀 Đang đồng bộ Ghi chú & Bài viết lên Google Sheet...');
  try {
    const result = await saveAllNotesToSheet(NOTES_DATA);
    console.log(`✅ Đã đồng bộ thành công ${result.count} bài viết ghi chú lên Google Sheet tab "GhiChu"!`);

    const verified = await fetchNotesFromSheet();
    console.log(`📋 Kiểm tra lại: Đã đọc được ${verified.length} bài viết từ Google Sheet:`);
    verified.forEach((n, idx) => {
      console.log(`  ${idx + 1}. [${n.code}] ${n.title} (${n.category})`);
    });
  } catch (err) {
    console.error('❌ Lỗi đồng bộ với Google Sheets:', err);
    process.exit(1);
  }
}
